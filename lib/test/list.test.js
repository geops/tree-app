...
describe('invalid function parameters', () => {
  test('missing forestType', () =>
    expect(() => listUtils.list()).toThrowError(`forestType is missing`));

  test('invalid forestType', () =>
    expect(() => listUtils.list({ forestType: 'fooBar' })).toThrowError(
      `fooBar is not valid`,
    ));

  test('invalid transitionForestType', () =>
    expect(() =>
      listUtils.list({ forestType: '60', transitionForestType: 'fooBar' }),
    ).toThrowError(`fooBar is not valid`));
});

describe('valid results', () => {
  test('valid forestType', () =>
    expect(listUtils.list({ forestType: '60' })).toStrictEqual([
      [302800],
      [402300],
  ...
  test('nonresident tree types (330600, 346500) at the end of list and 9500 only in attention', () =>
    expect(listUtils.list({ forestType: '55 collin' })).toStrictEqual([
      [335900],
      [],
  ...
  test('valid forestType and transitionForestType', () =>
    expect(
      listUtils.list({ forestType: '60', transitionForestType: '55 collin' }),
