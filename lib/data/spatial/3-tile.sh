_tile () {
    local TARGET=$1
    echo "Tiling ${TARGET} ..."
    tippecanoe --simplify-only-low-zooms --simplification=5 --detect-shared-borders --no-feature-limit --include=code --include=code_style --include=code_vd --include=info_vd --include=code_so --minimum-zoom=0 --maximum-zoom=12 -s EPSG:3857 --force -o "/data/spatial/tiles/${TARGET}.mbtiles" /data/spatial/export/${TARGET}.geojson
}
