_tile () {
    ...
    echo "Tiling ${TARGET} ..."
    tippecanoe --no-tile-size-limit --detect-shared-borders --no-feature-limit --include=code --include=code_style --include=code_vd --include=info_vd --include=code_so --minimum-zoom=${MIN_ZOOM} --maximum-zoom=${MAX_ZOOM} -s EPSG:3857 --layer ${LAYER_NAME} --force -o "/data/spatial/tree-app-tiles/tiles/${TARGET}.mbtiles" /data/spatial/export/${TARGET}.geojson
}
...
_tile "forest_types" "forest_types" 14 12
_tile "forest_types_union" "forest_types" 11 0
...
rm -f altitudinal_zones_1995.mbtiles altitudinal_zones_2085_dry.mbtiles altitudinal_zones_2085_less_dry.mbtiles cantonal_boundaries.mbtiles forest_ecoregions.mbtiles forest_types.mbtiles silver_fir_areas.mbtiles forest_types_union.mbtiles
