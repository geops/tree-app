_tile () {
    local TARGET=$1
    echo "Tiling ${TARGET} ..."
    tippecanoe --detect-shared-borders --no-feature-limit --no-tile-size-limit --include=code --include=code_style --minimum-zoom=0 --maximum-zoom=12 -s EPSG:3857 --force -o "/data/spatial/tiles/${TARGET}.mbtiles" /data/spatial/export/${TARGET}.geojson
}
