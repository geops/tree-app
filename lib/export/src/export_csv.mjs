...
function processData(err, rows) {
  ...
  const processed = data.reduce((finalOutputRows, row, idx) => {
    ...
    const location = reduceHochmontanLocation(
      inputLocation,
      inputLocation.altitudinalZone,
    );
    location.silverFirArea = reduceHochmontanSilverFir(
      location.targetAltitudinalZoneModerate,
      location.silverFirArea,
    );
    location.targetAltitudinalZoneModerate = reduceHochmontanAz(
      location.targetAltitudinalZoneModerate,
    );
    location.silverFirArea = reduceHochmontanSilverFir(
    ...
    location.targetAltitudinalZoneExtreme = reduceHochmontanAz(
      location.targetAltitudinalZoneExtreme,
    );
    ...
    let outputRowTemplate = {
      ...
      X95: x,
      Y95: y,
      Region: info('forestEcoregion', location.forestEcoregion)?.de,
  ...
  stringify(processed, { header: true }, function (err, output) {
    const filePath = `${process.cwd()}/src/exportedTreeRecommendations.csv`;
