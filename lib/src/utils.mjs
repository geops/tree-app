...
const getProjectionResultKey = (location, projectionResult, scenarioString) => {
  const projection = projectionResult?.[scenarioString];
  if (
    projection &&
    (!projection.projections || !projection.projections.slice(-1)[0]) &&
    location.altitudinalZone !==
      location[`targetAltitudinalZone${capitalize(scenarioString)}`]
  ) {
    /**
