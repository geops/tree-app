...
function locate(location = {}) {
  ...
  if (location.indicators && location.indicators.length > 0) {
    ...
    forestTypes = forestTypes.filter((ft) =>
      indicatorsForestTypes
        .map((ift) => ift.includes(ft.code))
        .every((includes) => includes === true),
    );
  }
  if (location.treeTypes && location.treeTypes.length > 0) {
    const treeTypesForestTypes = location.indicators.map(
      (ltt) => types.treeType.find((ttt) => ttt.code === ltt).forestTypes,
    );
    forestTypes = forestTypes.filter((ft) =>
      treeTypesForestTypes
        .map((ttft) => ttft.includes(ft.code))
        .every((includes) => includes === true),
    );
  }
  ...
  if (location.slopes) {
    forestTypes = forestTypes.filter(
      (ft) => intersection(ft.slope, location.aspects).length > 0,
    );
  }
