...
import { forestType, treeType } from '../data/types.json';

const emptyLists = [[], [], [], []];

const findForestType = (c) => forestType.find((f) => f.code === c);
...
const nonresidents = (c) => treeType.find((t) => t.code === c).nonresident;

const residents = (c) => !treeType.find((t) => t.code === c).nonresident;

export const sort = (lists) =>
  lists.map(
    (trees) =>
      trees
        .filter(residents)
