 ...
 */
function locate(location = {}, profile = 'ch') {
  if (!locations[profile]) {
    throw new Error(`${profile} is not a valid profile.`);
  }

  const options = {
    forestEcoregion: ch.forestEcoregion.map((fe) => fe.code),
  };
  const { forestEcoregion, altitudinalZone, silverFirArea } = location;
  let ecogram;
  if (locations[profile][forestEcoregion]) {
    options.altitudinalZone = Object.keys(locations[profile][forestEcoregion]);
    if (locations[profile][forestEcoregion][altitudinalZone]) {
      options.silverFirArea = Object.keys(
        locations[profile][forestEcoregion][altitudinalZone],
      );
      const id =
        locations[profile][forestEcoregion][altitudinalZone][silverFirArea] ||
        locations[profile][forestEcoregion][altitudinalZone].unknown;
      if (id) {
        ecogram = ecograms[profile][id];
      }
    }
  }

  let forestTypes = ch.forestType;
  ...
  if (location.indicators && location.indicators.length > 0) {
    const indicatorsForestTypes = location.indicators.map(
      (li) => ch.indicator.find((it) => it.code === li).forestTypes,
    );
    forestTypes = forestTypes.filter((ft) =>
  ...
  if (location.treeTypes && location.treeTypes.length > 0) {
    const treeTypesForestTypes = location.treeTypes.map(
      (ltt) => ch.treeType.find((ttt) => ttt.code === ltt).forestTypes,
    );
    forestTypes = forestTypes.filter((ft) =>
  ...
  options.indicator = ch.indicator.filter(byForestTypes).map((i) => i.code);
  options.treeType = ch.treeType.filter(byForestTypes).map((tt) => tt.code);
